...
plotGAM2<-function (gamFit, smooth.cov, groupCovs = NULL, orderedAsFactor = T, 
                    rawOrFitted = F, plotCI = T) 
{
  ...
  if (base::is.null(groupCovs)) {
    ...
    if (plotCI) {
      plot <- (ggplot2::ggplot(data = plot.df, ggplot2::aes(x = plot.df[, 
                 ...
                 ggplot2::ggtitle(base::paste0(base::as.character(old.formula)[2], 
                                               "  vs. s(", smooth.cov, ")")) + 
                 ggplot2::ylab(base::paste0("Predicted ", 
    ...
    temp.fm <- strsplit(old.covariates, split = "+", 
                        fixed = T)[[1]]
            ...
            if (grepl("=", temp.term[j])) {
              if (grepl("by", temp.term[j])) {
                ...
                if (all(order == 1)) {
                  ...
                  order[2] <- j
                  temp.term <- temp.term[order]
                  temp.term <- gsub(pattern = ")", 
                                    replacement = "", x = temp.term)
    ...
    if (!base::grepl(pattern = main.smooth, old.covariates, 
      ...
      if (plotCI) {
        plot <- (ggplot2::ggplot(data = plot.df, ggplot2::aes(x = plot.df[, 
                                                                          1])) + ggplot2::geom_line(ggplot2::aes(y = fit, 
                                                                                                                 col = factor(group)), linetype = factor(group), size = 1) + ggplot2::geom_ribbon(data = plot.df, 
                                                                                                                                                                        ggplot2::aes(ymax = fit + 1.96 * se.fit, ymin = fit - 
                                                                                                                                                                                       1.96 * se.fit, col = factor(group), linetype = factor(group)), 
                   ...
                   ggplot2::scale_linetype_discrete(name = groupCovs)+
                   ggplot2::theme_bw())
      }
      else {
        plot <- (ggplot2::ggplot(data = plot.df, ggplot2::aes(x = plot.df[, 
                                                                          1])) + ggplot2::geom_line(ggplot2::aes(y = fit, 
                                                                                                                 col = factor(group)), size = 1, linetype = factor(group)) + ggplot2::ggtitle(base::paste0(base::as.character(old.formula)[2], 
                                                                                                                                                                                 " vs. s(", smooth.cov, ")")) + 
                   ggplot2::ylab(base::paste0("Predicted ", 
                                              base::as.character(old.formula)[2])) + ggplot2::xlab(base::paste0("s(", 
                                                                                                                smooth.cov, ")")) 
      ...
      if (rawOrFitted == "raw") {
        plot <- plot + ggplot2::geom_point(data = temp.data, 
                                           ggplot2::aes(x = purrr::as_vector(temp.data[smooth.cov]), 
                                                        y = temp.data[, 1],linetype = temp.data[groupCovs][, #col = temp.data[groupCovs][, 
                                                                                                           1]))
        base::return(plot)
      }
      else if (rawOrFitted == "fitted") {
        ...
        plot <- plot + ggplot2::geom_point(data = temp.data, 
                                           ggplot2::aes(x = purrr::as_vector(temp.data[smooth.cov]), 
                                                        y = fitted, linetype = temp.data[groupCovs][, #col = temp.data[groupCovs][, 
                                                                                                    1]))
        base::return(plot)
      }
      else {
      ...
      if (base::class(temp.data[groupCovs][, 1])[1] == 
        ...
        temp.data[groupCovs] <- as.factor(as.character(temp.data[groupCovs][, 
                                                                            1]))
        textCall <- gam$call
        temp.fm.updated <- strsplit(old.covariates, split = "+", 
                                    fixed = T)[[1]]
        index.Main <- setdiff(grep(pattern = main.smooth, 
                                   x = temp.fm.updated, fixed = T), grep(pattern = interaction.smooth, 
                                                                         x = temp.fm.updated, fixed = T))
        temp.fm.updated <- temp.fm[-index.Main]
        updatedCovs <- paste0(temp.fm.updated, collapse = "+")
        updatedCovs <- base::gsub(" ", "", 
                                  updatedCovs)
      ...
      if (plotCI) {
        ...
        plot <- (ggplot2::ggplot(data = plot.df, ggplot2::aes(x = plot.df[, 
                                                                          1])) + ggplot2::geom_line(ggplot2::aes(y = fit, 
                                                                                                                 col = factor(group), linetype = factor(group)), size = 1) + ggplot2::ggtitle(base::paste0(base::as.character(old.formula)[2], 
                                                                                                                                                                                 "  vs. s(", smooth.cov, ")")) + 
                   ggplot2::ylab(base::paste0("Predicted ", 
                                              base::as.character(old.formula)[2])) + ggplot2::xlab(base::paste0("s(", 
                                                                                                                smooth.cov, ")")) 
